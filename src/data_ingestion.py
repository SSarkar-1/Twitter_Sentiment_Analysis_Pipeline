...
def load_data(data_url: str) -> pd.DataFrame:
    try:
        # Read the CSV file - it has all data in a single column
        df = pd.read_csv(data_url)
        
        # Split the single column into separate columns
        df_split = df['tweet_id,sentiment,content'].str.split(',', n=2, expand=True)
        df_split.columns = ['tweet_id', 'sentiment', 'content']
...
def main():
    try:
        df = load_data(data_url='https://raw.githubusercontent.com/SSarkar-1/Sentiment_Analysis_Pipeline/refs/heads/main/data.csv')
